...
def get_categories(context, num=10):
    ...
    try:
        user = context['user']
        user = context['user']
        return Category.objects.filter(article__author=user).annotate(num_posts=Count('article')).filter(
            num_posts__gt=0)[:num]
...
def get_tags(context, num=10):
    ...
    try:
        ...
        return Tag.objects.filter(article__author=user).annotate(num_posts=Count('article')).filter(
            num_posts__gt=0)[:num]
